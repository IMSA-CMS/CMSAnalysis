...
class NanoAODEventFile : public EventFile
{
  public:
    ...
    virtual int getNumOfEvents() const override
    {
    ...
    virtual bool checkTrigger(std::string triggerName,
                              std::string subProcess) const override; // checks if event passes trigger criteria
  ...
  private:
    ...
    template <typename T> class TreeVariable : public TreeVariableBase
    {
      public:
        ...
        {
            return std::static_pointer_cast<ROOT::Internal::TTreeReaderValueBase>(
                std::make_shared<T>(reader, branchName.c_str()));
        }
    ...
    template <typename T> T getArrayElement(std::string name, int index) const
    {
    ...
    template <typename T> unsigned getArraySize(std::string name) const
    {
        auto it = variables.find(name);
        if (it == variables.end())
        {
    ...
    template <typename T> T getVariable(std::string name) const
    {
        auto it = variables.find(name);
        if (it == variables.end())
        {
