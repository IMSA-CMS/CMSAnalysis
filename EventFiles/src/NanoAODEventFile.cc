...
bool NanoAODEventFile::checkTrigger(std::string triggerName, std::string subProcess) const
{
    ...
    if (trigger == triggers.end())
    {
        ...
        treeReader.Next();
        // call Restart() which calls next but goes back to first entry, then we go
        // to SetEntry again then treeReader.Next again, do this after setTree
    }
    return *(trigger->second);
}

NanoAODEventFile::NanoAODEventFile(TFile *ifile, std::shared_ptr<FileParams> iparams) : EventFile(ifile, iparams)
{
    ...
    std::vector<std::shared_ptr<TreeVariableBase>> treeVariables = {
        ...
        // std::make_shared<TreeVariable<TTreeReaderArray<Float_t>>>("elec_dEscaleDown",
        // "Electron_dEscaleDown"),
        // std::make_shared<TreeVariable<TTreeReaderArray<Float_t>>>("elec_dEscaleUp",
        // "Electron_dEscaleUp"),
        // std::make_shared<TreeVariable<TTreeReaderArray<Float_t>>>("elec_dEsigmaDown",
        // "Electron_dEsigmaDown"),
        // std::make_shared<TreeVariable<TTreeReaderArray<Float_t>>>("elec_dEsigmaUp",
        // "Electron_dEsigmaUp"),
    ...
    // "nElectron"),
    // std::make_shared<TreeVariable<TTreeReaderValue<UInt_t>>>("muon_size",
    // "nMuon"),
    // std::make_shared<TreeVariable<TTreeReaderValue<UInt_t>>>("photon_size",
    // "nPhoton"),
    // std::make_shared<TreeVariable<TTreeReaderValue<UInt_t>>>("jet_size",
    // "nJet"),
    // std::make_shared<TreeVariable<TTreeReaderValue<UInt_t>>>("gen_size",
    // "nGenPart"),
...
void NanoAODEventFile::nextEvent()
{
    ...
    if (variables.find("gen_size") != variables.end() && getVariable<UInt_t>("gen_size") > 0)
    {
        ...
        int mothersIndex;
        std::vector<std::vector<Int_t>> daughtersVectors(
            (int)getVariable<UInt_t>("gen_size"),
            std::vector<int>(0)); // Vector of Vectors, the first index represents the mother, the second vector has the
                                  // aughter indexes
        for (unsigned j = 0; j < getVariable<UInt_t>("gen_size"); j++)
        {
            ...
            {
                daughtersVectors[mothersIndex].push_back(j); // at daughtersVectors[mothersIndex], adds the current
                                                             // particle, making the current particle the daughter
        ...
        for (unsigned i = 0; i < getVariable<UInt_t>("gen_size"); i++) // Loops through every particle
        {
            ...
            for (auto index : daughtersVectors[i]) // for every particle, go throught its daughters
                                                   // and add add its index in genSimp ro daughter
                                                   // collection vector
            {
                ...
                // gensim is cleared every time, so its always empty, so the daughter
                // thing is pointing at garbage?, probably not

                // std::cout<< "gensimindex: " << &genSimParticles[index] << "\n";
                // std::cout<< "daughter collection index" <<
                // daughterCollectionVector[0] << "\n";                 //index and
                // vector addresses are the same
            }
            ...
            genSimParticles.push_back(
                GenSimParticle(reco::Candidate::LorentzVector(math::PtEtaPhiMLorentzVector(
                               ...
                               getArrayElement<Int_t>("gen_pid", i), mother, daughterCollectionVector,
                               getArrayElement<Int_t>("gen_status", i)));

            if (daughterCollectionVector.size() > 0)
            {
                // std::cout<< "GenSimParticle " <<
                // &genSimParticles[daughtersVectors[i][0]] << "\n"; std::cout<<
                // "Daughter zero: " << daughterCollectionVector[0] << "\n";
                if (&genSimParticles[daughtersVectors[i][0]] != daughterCollectionVector[0])
                {
                    // std::cout << "ERROR!" << "\n";
                }

                // std::cout<< "Daughter: " << daughterCollectionVector[0]->getName();
                // //segfaulted
            }
        }
    }
    ...
    for (UInt_t i = 0; i < getVariable<UInt_t>("elec_size"); i++)
    {
        ...
        // std::make_shared<TreeVariable<TTreeReaderArray<Bool_t>>>("mvaElecWP80",
        // "MVA Iso ID V2 WP80 Electron_mvaFall17V2Iso_WP80"),
        // std::make_shared<TreeVariable<TTreeReaderArray<Bool_t>>>("mvaElecWP90",
        // "Electron_mvaFall17V2Iso_WP90"),
        // std::make_shared<TreeVariable<TTreeReaderArray<Bool_t>>>("mvaElecWPL",
        // "Electron_mvaFall17V2Iso_WPL"),

        Particle::SelectionFit fit;
        auto elecIdpass = getArrayElement<Int_t>("elec_idpass", i);
        if (elecIdpass == 4)
        {
            fit = Particle::SelectionFit::Tight;
        }
        else if (elecIdpass == 3)
        {
            fit = Particle::SelectionFit::Medium;
        }
        else if (elecIdpass == 2)
        {
            fit = Particle::SelectionFit::Loose;
        }
        else
        ...
        // Lorentz four-vector
        auto elecDz = getArrayElement<Float_t>("elec_dz", i);
        auto elecDxy = getArrayElement<Float_t>("elec_dxy", i);

        auto particle =
            Particle(reco::Candidate::LorentzVector(math::PtEtaPhiMLorentzVector(
                         getArrayElement<Float_t>("elec_pt", i), getArrayElement<Float_t>("elec_eta", i),
                         getArrayElement<Float_t>("elec_phi", i), getArrayElement<Float_t>("elec_mass", i))),
                     elecDxy, elecDz, charge, ParticleType::electron(), fit);
        // std::cout << "NanoAOD: " << getArrayElement<Bool_t>("elec_cutBasedHEEP",
        // i) << '\n';
        //        particle.addInfo("CutBasedHEEP",
        //        getArrayElement<Bool_t>("elec_cutBasedHEEP", i));
        ...
        particle.addInfo("dxy", elecDxy);
        particle.addInfo("dz", elecDz);
        // particle.addInfo("eScaleDown",
        // getArrayElement<Float_t>("elec_dEscaleDown", i));
        // particle.addInfo("eScaleUp", getArrayElement<Float_t>("elec_dEscaleUp",
        // i)); particle.addInfo("eSigmaDown",
        // getArrayElement<Float_t>("elec_dEsigmaDown", i));
        // particle.addInfo("eSigmaUp", getArrayElement<Float_t>("elec_dEsigmaUp",
        // i));
    ...
    for (UInt_t i = 0; i < getVariable<UInt_t>("muon_size"); i++)
    {
        ...
        auto muonMvaid = getArrayElement<UChar_t>("muon_mvaid", i);
        if (muonMvaid >= 3)
        {
            fit = Particle::SelectionFit::Tight;
        }
        ...
        auto muonDz = getArrayElement<Float_t>("muon_dz", i);
        auto muonDxy = getArrayElement<Float_t>("muon_dxy", i);

        auto particle =
            Particle(reco::Candidate::LorentzVector(math::PtEtaPhiMLorentzVector(
                         getArrayElement<Float_t>("muon_pt", i), getArrayElement<Float_t>("muon_eta", i),
                         getArrayElement<Float_t>("muon_phi", i), getArrayElement<Float_t>("muon_mass", i))),
                     muonDxy, muonDz, charge, ParticleType::muon(), fit);

        particle.addInfo("Isolation", getArrayElement<Float_t>("muon_reliso", i));
        particle.addInfo("dxy", muonDxy);
        particle.addInfo("dz", muonDz);
        recoParticles.addParticle(particle);
    }
    for (UInt_t i = 0; i < getVariable<UInt_t>("photon_size"); i++)
    {
        ...
        auto particle =
            Particle(reco::Candidate::LorentzVector(math::PtEtaPhiMLorentzVector(
                         getArrayElement<Float_t>("photon_pt", i), getArrayElement<Float_t>("photon_eta", i),
                         getArrayElement<Float_t>("photon_phi", i), 0)),
...
ParticleCollection<Particle> NanoAODEventFile::getRecoJets() const
{
    ...
    {
        Particle particle(reco::Candidate::LorentzVector(
                              getArrayElement<Float_t>("jet_pt", i), getArrayElement<Float_t>("jet_eta", i),
