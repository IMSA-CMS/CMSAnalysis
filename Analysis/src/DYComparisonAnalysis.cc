...
//                 newDY->addProcess(makeBasicProcess(histVariables, newFilePath, "newDY50.root", "dy50toinf", reader, massTarget, luminosity));
//             auto oldDY = std::make_shared<Process>("Delphes", 2);
//             oldDY->addProcess(makeBasicProcess(histVariables, newFilePath, "DY50.root", "dy50toinf", reader, massTarget, luminosity));
...
// TH1* DYComparisonAnalysis::getHist(std::string histType, std::string processName, double massTarget, bool scaleToExpected, std::string channelName) const {
//     int maxBinNum = 0;
