...
void Channel::labelProcess(Label label, std::shared_ptr<Process> process)
{
...
void Channel::CombineDatacard(HistVariable histType)
{
    ...
    {
        auto processName = Utility::substitute(process->getName(), " ", "_");
        datacard << "shapes" << std::setw(20) << processName << std::setw(20) << channelName << std::setw(40)
                 << "Genhiggsworkspace.root  " << "higgsworkspace.root:" << processName << "\n";
    }
    datacard << "shapes				data_obs" << std::setw(20) << channelName << std::setw(50)
             << "Genhiggsworkspace.root Genhiggsworkspace:" << channelName << "Events\n";
...
void Channel::addProcessLabel(Label label, std::vector<std::shared_ptr<Process>> processes)
{
                           ...
                           int rebinConstant) const
{
    ...
    {
        for (const auto &process : map.find(label.value())->second)
        {
            TH1 *hist = process->getHist(histType, scaleToExpected);
            if (label.value() == Label::Signal)
            {
...
std::vector<TH1 *> Channel::getHists(HistVariable histType, std::optional<Label> label, bool scaleToExpected) const
{
