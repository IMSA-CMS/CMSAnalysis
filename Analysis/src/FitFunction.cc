...
if (!systematics.empty())
{
    ...
    {
        TF1* upFunc = function.getSystematic(sysName, true);
        TF1* downFunc = function.getSystematic(sysName, false);
...
void FitFunction::addSystematic(const std::string& sysName, TF1* upFunction, TF1* downFunction)
{
    if (!upFunction || !downFunction)
    {
        std::cerr << "[FitFunction] Warning: Null TF1 passed to addSystematic for " << sysName << std::endl;
        return;
    }
...
void FitFunction::addSystematic(const std::string& sysName, const std::vector<double>& upParams, const std::vector<double>& downParams)
{
    if (!function)
    {
        std::cerr << "[FitFunction] Error: Base function not set before adding systematic: " << sysName << std::endl;
        return;
    }
...
TF1* FitFunction::getSystematic(const std::string& sysName, bool up) const
{
