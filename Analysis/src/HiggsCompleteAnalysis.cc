    ...
    for (const std::string &recoDecay : recoDecays)
    {
        for (const auto &genSimDecay : genSimDecays)
        {
            ...
            for (double massTarget : massTargets)
            {
                ...
                // histVariableToFileMapping["Same Sign Invariant Mass"] = decayName + "__hists/" + decayName +
                // "_Reco Same Sign Invariant Mass"; histVariableToFileMapping["X Projection"] = decayName +
                // "__hists/" + decayName + "_Reco Invariant Mass Background X Projection";
                // histVariableToFileMapping["Y Projection"] = decayName + "__hists/" + decayName + "_Reco Invariant
                // Mass Background Y Projection";
                std::vector<HistVariable> histVariablesSignal;

                // histVariablesSignal.push_back(HistVariable::sameSignMass(decayName + "__hists/" + decayName +
                ...
                auto higgsProcess = makeBasicProcess(histVariablesSignal, signalFilePath,
                ...
                auto higgsSignal =
                    std::make_shared<Process>("Higgs signal " + decayName + " " + std::to_string((int)massTarget), 1);
            ...
            zzBackground->addProcess(makeBasicProcess(histVariablesBackground, filePath, "ZZ.root", "zzto4l", reader,
                                                      luminosity, histVariableToFileMapping));

            // auto zzBackgroundNoVeto = std::make_shared<Process>("ZZ Background No Veto", 3);
            // zzBackgroundNoVeto->addProcess(makeBasicProcess(histVariablesBackground,
            // "/uscms/homes/v/vyou/analysis/CMSSW_14_0_4/src/CMSAnalysis/Output/NoZVeto/", "ZZ.root", "zzto4l", reader,
            // luminosity, histVariableToFileMapping));

            auto wJetsBackground = std::make_shared<Process>("WJets Background", WJetsBackgroundColor);
            wJetsBackground->addProcess(makeBasicProcess(histVariablesBackground, filePath, "WJets.root", "WJets",
                                                         reader, luminosity, histVariableToFileMapping));
            ...
            ttBarandMultiBosonBackground->addProcess(makeBasicProcess(histVariablesBackground, filePath, "TTbar.root",
                                                                      "ttbar_lep", reader, luminosity,
                                                                      histVariableToFileMapping));
            ttBarandMultiBosonBackground->addProcess(makeBasicProcess(
                histVariablesBackground, filePath, "TTW.root", "ttw", reader, luminosity, histVariableToFileMapping));
            ttBarandMultiBosonBackground->addProcess(makeBasicProcess(
                histVariablesBackground, filePath, "TTZ.root", "ttz", reader, luminosity, histVariableToFileMapping));

            auto ttbarProcess = std::make_shared<Process>("TTbar Background", ttbarColor);
            ttbarProcess->addProcess(makeBasicProcess(histVariablesBackground, filePath, "TTbar.root", "ttbar_lep",
                                                      reader, luminosity, histVariableToFileMapping));

            auto ttzProcess = std::make_shared<Process>("TTZ Background", ttbarColor);
            ttzProcess->addProcess(makeBasicProcess(histVariablesBackground, filePath, "TTZ.root", "ttz", reader,
                                                    luminosity, histVariableToFileMapping));

            auto other = std::make_shared<Process>("Other Background", ttbarColor);
            other->addProcess(makeBasicProcess(histVariablesBackground, filePath, "TTW.root", "ttw", reader, luminosity,
                                               histVariableToFileMapping));
            other->addProcess(makeBasicProcess(histVariablesBackground, filePath, "ZZZ.root", "zzz", reader, luminosity,
                                               histVariableToFileMapping));
            other->addProcess(makeBasicProcess(histVariablesBackground, filePath, "WW.root", "wwto2l2nu", reader,
                                               luminosity, histVariableToFileMapping));
            other->addProcess(makeBasicProcess(histVariablesBackground, filePath, "WWW.root", "www", reader, luminosity,
                                               histVariableToFileMapping));
            other->addProcess(makeBasicProcess(histVariablesBackground, filePath, "WWZ.root", "wwz", reader, luminosity,
                                               histVariableToFileMapping));
            other->addProcess(makeBasicProcess(histVariablesBackground, filePath, "WZ.root", "wzto3lnu", reader,
                                               luminosity, histVariableToFileMapping));
            other->addProcess(makeBasicProcess(histVariablesBackground, filePath, "WZZ.root", "wzz", reader, luminosity,
                                               histVariableToFileMapping));

            auto dyBackground = std::make_shared<Process>("Drell-Yan Background", drellYanBackColor);
            dyBackground->addProcess(makeBasicProcess(histVariablesBackground, filePath, "DY10-50.root", "dy10to50",
                                                      reader, luminosity, histVariableToFileMapping));
            dyBackground->addProcess(makeBasicProcess(histVariablesBackground, filePath, "DY50-inf.root", "dy50toInf",
                                                      reader, luminosity, histVariableToFileMapping));

            // auto dyBackgroundNoVeto = std::make_shared<Process>("Drell-Yan Background No Veto", 2);
            // dyBackgroundNoVeto->addProcess(makeBasicProcess(histVariablesBackground,
            // "/uscms/homes/v/vyou/analysis/CMSSW_14_0_4/src/CMSAnalysis/Output/NoZVeto/", "DY10-50.root", "dy10to50",
            // reader, luminosity, histVariableToFileMapping));
            // dyBackgroundNoVeto->addProcess(makeBasicProcess(histVariablesBackground,
            // "/uscms/homes/v/vyou/analysis/CMSSW_14_0_4/src/CMSAnalysis/Output/NoZVeto/", "DY50-inf.root",
            // "dy50toInf", reader, luminosity, histVariableToFileMapping));

            auto qcdBackground = std::make_shared<Process>("QCD Background", QCDBackColor);
            qcdBackground->addProcess(makeBasicProcess(histVariablesBackground, filePath, "QCD100-200.root",
                                                       "QCD_100-200", reader, luminosity, histVariableToFileMapping));
            qcdBackground->addProcess(makeBasicProcess(histVariablesBackground, filePath, "QCD200-300.root",
                                                       "QCD_200-300", reader, luminosity, histVariableToFileMapping));
            qcdBackground->addProcess(makeBasicProcess(histVariablesBackground, filePath, "QCD300-500.root",
                                                       "QCD_300-500", reader, luminosity, histVariableToFileMapping));
            qcdBackground->addProcess(makeBasicProcess(histVariablesBackground, filePath, "QCD500-700.root",
                                                       "QCD_500-700", reader, luminosity, histVariableToFileMapping));
            qcdBackground->addProcess(makeBasicProcess(histVariablesBackground, filePath, "QCD700-1000.root",
                                                       "QCD_700-1000", reader, luminosity, histVariableToFileMapping));
            qcdBackground->addProcess(makeBasicProcess(histVariablesBackground, filePath, "QCD1000-1500.root",
                                                       "QCD_1000-1500", reader, luminosity, histVariableToFileMapping));
            qcdBackground->addProcess(makeBasicProcess(histVariablesBackground, filePath, "QCD1500-2000.root",
                                                       "QCD_1500-2000", reader, luminosity, histVariableToFileMapping));
            // qcdBackground->addProcess(makeBasicProcess(histVariablesBackground, filePath, "QCD2000-inf.root",
            // "QCD_2000-inf", reader, luminosity, histVariableToFileMapping));
            ...
            higgsData->addProcess(makeBasicProcess(histVariablesBackground, filePath, "Electron2016.root",
                                                   "Electron2016", reader, luminosity, histVariableToFileMapping,
                                                   true));
            higgsData->addProcess(makeBasicProcess(histVariablesBackground, filePath, "Electron2016APV.root",
                                                   "Electron2016APV", reader, luminosity, histVariableToFileMapping,
                                                   true));
            higgsData->addProcess(makeBasicProcess(histVariablesBackground, filePath, "Electron2017.root",
                                                   "Electron2017", reader, luminosity, histVariableToFileMapping,
                                                   true));
            higgsData->addProcess(makeBasicProcess(histVariablesBackground, filePath, "Electron2018.root",
                                                   "Electron2018", reader, luminosity, histVariableToFileMapping,
                                                   true));
            higgsData->addProcess(makeBasicProcess(histVariablesBackground, filePath, "Muon2016.root", "Muon2016",
                                                   reader, luminosity, histVariableToFileMapping, true));
            higgsData->addProcess(makeBasicProcess(histVariablesBackground, filePath, "Muon2016APV.root", "Muon2016APV",
                                                   reader, luminosity, histVariableToFileMapping, true));
            higgsData->addProcess(makeBasicProcess(histVariablesBackground, filePath, "Muon2017.root", "Muon2017",
                                                   reader, luminosity, histVariableToFileMapping, true));
            higgsData->addProcess(makeBasicProcess(histVariablesBackground, filePath, "Muon2018.root", "Muon2018",
                                                   reader, luminosity, histVariableToFileMapping, true));
