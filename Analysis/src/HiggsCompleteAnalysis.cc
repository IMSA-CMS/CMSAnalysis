...
HiggsCompleteAnalysis::HiggsCompleteAnalysis() {
    ...
    //const std::vector<std::string> genSimDecays{"eeeu", "eueu"};
    //const std::vector<std::string> recoDecays{"eeeu", "eueu"};
    ...
    for (std::string recoDecay : recoDecays){
        for(double massTarget : massTargets) {
            ...
            auto dyBackground = std::make_shared<Process>("γ/Z -> e⁺e⁻ Background", 2);
            ...
            higgsData->addProcess(makeSignalProcess(histVariablesBackground, filePath, "Data_Trigger_SingleMuon_Year_2017a.root", "higgs4l" + std::to_string((int) massTarget), reader, massTarget, luminosity));
