...
double WindowEstimator::getExpectedYield(const SingleProcess* process, double luminosity) const
{
    ...
    TH2 *histanalysis = (TH2F *)process->getHist(); //Change this for different hist analysis
    ...
    double analysisYhighboundbin = histanalysis->GetYaxis()->FindBin(masshighaccepted);
    //std::cout << "high bin " << analysisupboundbin << std::endl;

    double fraction4l = (histanalysis->Integral(analysisXlowboundbin, analysisXupboundbin, analysisYlowboundbin, analysisYhighboundbin) / totalanalysishistintegral);
