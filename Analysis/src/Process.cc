...
TH1 *Process::getHist(const HistVariable &histType, bool scaleToExpected) const
{
    ...
    auto *newHist = new TH1F(name.c_str(), name.c_str(), maxBinNum, 0, maxBinNum * maxBarWidth);
    auto *toMerge = new TList;
    for (const auto &singleProcess : processes)
    {
    ...
    // If you want yield to print while running SuperPlot uncomment the print statement (only prints the yield for the
    // first MassTarget in the process) std::cout << "Total yield for mass target " << processes.at(0).getMassTarget()
    // << " is " << getYield("processes.at(0).getMassTarget()") << std::endl; std::cout << "Process Databin of " <<
    // newHist->GetName() << " is: " << newHist->GetEntries() << std::endl;
    return newHist;
}
...
TH2 *Process::get2DHist(const HistVariable &histType) const
{
    ...
    for (const auto &singleProcess : processes)
    {
        ...
        maxBinNum = std::max(singleProcess.get2DHist(histType)->GetNbinsX(), maxBinNum);
        ...
        yMaxBinNum = std::max(singleProcess.get2DHist(histType)->GetNbinsY(), yMaxBinNum);
    ...
    auto *hist = new TH2F(name.c_str(), name.c_str(), maxBinNum, 0, maxBinNum * xMaxBarWidth, yMaxBinNum, 0,
                          yMaxBinNum * yMaxBarWidth);
    auto *toMerge = new TList;
    for (const auto &singleProcess : processes)
    {
    ...
    // If you want yield to print while running SuperPlot uncomment the print statement (only prints the yield for the
    // first MassTarget in the process) std::cout << "Total yield for mass target " << processes.at(0).getMassTarget()
    // << " is " << getYield(processes.at(0).getMassTarget()) << std::endl;
    return hist;
}

TH1 *Process::getSingleProcessHist(const HistVariable &histType, const std::string &singleProcessName,
                                   bool scaleToExpected) const
{
...
{
    systematics.addSystematic(std::move(systematic));
}
...
std::pair<TH1 *, TH1 *> Process::combineSystematics(std::vector<std::shared_ptr<Process>> processes, TH1 *original,
...
std::shared_ptr<Systematic> Process::calcRateSystematic(HistVariable histType, std::string systematicName)
{
    ...
    if (!up || !down)
    {
        std::cerr << "Could not create rate systematic histograms for " << systematicName << " in process " << name
                  << "\n";
        return nullptr;
    }
    ...
    {
        std::cerr << "Missing histograms or zero integral in process: " << name << "\n";
        return nullptr;
    }
...
std::optional<FitFunction> Process::getPlot(const HistVariable &histType)
{
    ...
    {
        auto otherType = std::get<0>(entry);
        if (otherType == histType)
        {
...
void Process::setPlot(const HistVariable &histType, FitFunction plot)
{
    for (auto &entry : plots)
    {
        ...
        if (otherType == histType)
        {
            std::get<1>(entry) = std::move(plot);
            return;
        }
    }
    plots.push_back({histType, std::move(plot)});
}
