...
void multipleFits()
{
    ...
    //auto higgsAnalysis = std::make_shared<HiggsCompleteAnalysis>(); //works
    //auto higgsAnalysis = std::make_shared<HiggsChannelsAnalysis>(); //works
    //auto higgsAnalysis = std::make_shared<HiggsGenComparisonAnalysis>();//doesn't work
    ...
    {   
        for(std::string processName : channel->getNames()) {
            ...
            if(processName == "Higgs Signal") {
                auto SignalProcess = channel->findProcess(processName);
                TH1* hist = SignalProcess->getHist("Invariant Mass",false);
                ...
                std::string rootName(processName);
...
std::map<std::string, std::vector<std::vector<double>>> paramsFromFits(std::vector<std::vector<double>> results, std::vector<std::vector<double>> errors)
{
	...
	std::vector<double> x{300.0, 500.0, 700.0, 900.0, 1100.0, 1300.0, 1500.0, 1700.0};
