...
void SuperPlot()
{
	...
	*/
	...
	auto nameVar = HistVariable(HistVariable::VariableType::InvariantMass);
	auto ana = std::make_shared<DarkPhotonCompleteAnalysis>();
	TCanvas *canvas = plotFormatter->completePlot(ana, nameVar, xAxisTitle, yAxisTitle, true, false, true, "Low Mass and Different Sign/1jet");
