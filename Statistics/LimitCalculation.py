...
##########################need to complete
#def SplitCards(Case, WhichStudy):
######################################
...
def makeCards(cardName, infoName):
    ...
    #print(Signal)
    Plist = []
    for i in range (3,8):# was 3 to 10
    ...
    Text = Text + "kmax " + str("8") + " number of nuisance parameters\n" #the str should be NoUC
    Text = Text + "----------------------------------------------------------------------------------------------------------------------------------\n"
    Currentline = "bin          "
...
def runHCT(rMaxes, rMaxIndex):
    ###print("hello")
    Command = 'combine -M BayesianToyMC tempdatacard.txt -t 1 --noDefaultPrior=0 --rMax ' + str(rMaxes[rMaxIndex])
...
WhichStudy = input()
Automation = input("Enter M for manuel seperation of masses.  Enter A for automatic.\n-->")
if Automation == "A":
    #Automator = SplitCards(Case, WhichStudy) NEED TO IMPLEMENT!!!!!!!!!!!!!!!!!!!!!!!!!!!
#if WhichStudy == 1:
    #rMaxes = [0.1,0.1,0.3,1,2,5,10,15]#[0.1,1,5,10,50,100]
    #Case = input("Enter 1 for MuMu.  Enter 2 for EE.  Enter 3 for EMu.\n-->")
#else:
    rMaxes = [0.1,0.1,0.5]
rMaxIndex = 0
if WhichStudy == 1:
