...
for i in $(seq 30 10 100)
do
    echo "Current Mu pT cut: $i"
    pathName="DarkPhoton_MLEval_MuPtCut_NT_$i/"
    pathNameStr="DarkPhoton_MLEval_MuPtCut_NT_$i"
    ...
    #     runAnalyzer input="$file" output="${pathNameStr}${outputFile}" analysis=LeptonJetReconstruction numFiles=2 > nohup.out
    # done
