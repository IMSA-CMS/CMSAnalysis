...
{ 
  ...
  for (unsigned int i = 0; i < filenames.size(); i++)
      ...
      for (unsigned int j = 0; j < filenames[i].size(); j++)
	{
	  ...
	  if (!ifs)
	    throw std::runtime_error("File " + filenames[i][j] + " not found! Refer to the file, \"README.txt\", in the CMSAnalysis folder to make sure the inputs added into \"pickFiles.txt\" are valid.");
