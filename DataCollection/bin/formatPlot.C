...
TCanvas* finalPlot( int iPeriod, int iPos );
...
void formatPlot()
{
  ...
  // second parameter in finalPlot is iPos, which drives the position of the CMS logo in the plot
  ...
  //finalPlot( iPeriod, 0 );   // out of frame (in exceptional cases)
    finalPlot( iPeriod, 11 );  // left-aligned
  //  finalPlot( iPeriod, 33 );  // right-aligned

  //  writeExtraText = false;       // remove Preliminary
  
  //  finalPlot( iPeriod, 0 );   // out of frame (in exceptional cases)

  //  finalPlot( iPeriod, 11 );  // default: left-aligned
  //  finalPlot( iPeriod, 22 );  // centered
  //  finalPlot( iPeriod, 33 );  // right-aligned  
}
...
{ 
  ...
  h->GetXaxis()->SetTitle("Same sign invariant mass [GeV]");  
  h->GetYaxis()->SetNdivisions(6,5,0);
  h->GetYaxis()->SetTitleOffset(1.5);
  //h->GetYaxis()->SetTitle("m_{\\ell^{-}\\ell^{-}} \\text{(GeV)}");  
...
double func = std::numeric_limits<double>::max();
    ...
    while (true)
    {
        auto result = h->Fit(fitfunc, "RBS", "", 150, 1700);
