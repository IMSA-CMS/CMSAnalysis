...
void gammaAcceptance()
{
    ...
    string filteredHist = "Gen Sim Gamma (Reconstructed Jets)";
    string allHist = "Gen Sim Gamma";
    string title = "Gamma Acceptance";
...
void process(TH1* hist, std::string canvasName, std::string xaxis, std::string yaxis, std::string title){
    TFile* refinedOutput = TFile::Open("gammaAcceptance.root", "UPDATE");
