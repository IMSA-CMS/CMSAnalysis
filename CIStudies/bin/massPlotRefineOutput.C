...
void massPlotRefineOutput()
{
  gStyle->SetOptStat(0);
  TFile* testHiggs = TFile::Open("testHiggs.root");
  ...
  plotTwoPlots("3GenSim Opposite Sign Invariant Mass Pasted;1", "3Reco Opposite Sign Invariant Mass Pasted;1", "Opposite Sign Invariant Mass Distribution for Muon and Electron Events with 3 Matched Leptons", testHiggs, "Opposite Sign Invariant Mass", "Number of Events","GenSim", "Reco");
...
void plotTwoPlots(string hist1, string hist2, string canvasName, TFile* file1, string xaxis, string yaxis, string legend1, string legend2, double legendLeftSide)
{
  ...
  //c1->SetLogy(true);
  const char* canvasTitle = "Opposite Sign Invariant Mass Distribution for Muon and Electron Events with 3 Matched Leptons";
  ...
  // double plot4max = std::max(plot3max, plot4->GetMaximum());
  // double plot5max = std::max(plot4max, plot5->GetMaximum());
