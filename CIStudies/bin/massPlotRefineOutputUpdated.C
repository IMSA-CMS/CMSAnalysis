...
void plotTwoPlots(string hist1, string hist2, string hist3,  string hist4, string hist5, string canvasName, TFile* file1, TFile* file2, TFile* file3, TFile* file4, TFile* file5,  string xaxis, string yaxis, string legend1, string legend2, string legend3, string legend4, string legend5,  double legendLeftSide = .4);


void massPlotRefineOutputUpdated()
{
  gStyle->SetOptStat(0);
  TFile* genOutput = TFile::Open("genOutput.root");
  TFile* lambda24 = TFile::Open("lambda24.root");
  TFile* lambda32 = TFile::Open("lambda32.root");
  TFile* lambda40 = TFile::Open("lambda40.root");
  TFile* lambda100k = TFile::Open("lambda100k.root");
  ...
  TFile* outputPercentDiff = TFile::Open("genOutput.root", "recreate");

  plotTwoPlots("Reco Invariant Mass Pasted;1", "Reco Invariant Mass Pasted;1", "Reco Invariant Mass Pasted;1",  "Reco Invariant Mass Pasted;1",  "Reco Invariant Mass Pasted;1", "Invariant Mass Distribution for Lambda = 16, 24, 32, 40, and 100k Reco", genOutput, lambda24, lambda32, lambda40, lambda100k, "Invariant Mass", "Number of Events", "Lambda16RecoMuons","Lambda24RecoMuons", "Lambda32RecoMuons", "Lambda40RecoMuons", "Lambda100kRecoMuons");
...
void plotTwoPlots(string hist1, string hist2, string hist3, string hist4, string hist5, string canvasName, TFile* file1, TFile* file2, TFile* file3, TFile* file4, TFile* file5, string xaxis, string yaxis, string legend1, string legend2, string legend3, string legend4, string legend5, double legendLeftSide)
{
  ...
  double plot3max = plot3->GetMaximum();
  double plot4max = plot4->GetMaximum();
  double plot5max = plot5->GetMaximum();
  if (max < plot3max)
  {
	max = plot3max;
  }
  else if (max < plot4max)
  {
	max = plot4max;
  }
  else if (max < plot5max)
  {
  	max = plot5max;
  }
