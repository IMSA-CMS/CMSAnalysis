...
public:
  ...
  virtual bool
  process(const edm::EventBase &event) override; // Fills the histograms
  virtual void finalize() override;              // Scales the histograms
  void addHistogram(std::shared_ptr<HistogramPrototype> hist) {
    histograms.push_back(hist);
  }; // Adds a HistogramPrototype* to histogram (the vector)

protected:
  // This adds an object to the collection to be written.
  // All objects that use this method are duplicated for different filter
  // categories and written to a Root file by writeAll(). If you add two objects
  // of the same name, the second will replace the first.
  ...
  // takes care of that.  Simply code as though there was only one set of
  // histograms, and they will duplicate automatically as needed.
  ...
  // Creates a histogram and adds it to the collection.  Just a convenient
  // shortcut for addObject() above.
  void makeHistogram(const std::string &name, const std::string &title,
                     int nbins, double min, double max);
  // Creates a histogram and adds it to the collection, but uses the
  // HistogramPrototype* object void makeHistogram(HistogramPrototype* h);

  // Creates a histogram from a HistogramPrototype* and adds it to the
  // collection.
  ...
  TH1 *getHistogram(const std::string &name) {
    return dynamic_cast<TH1 *>(getObject(name));
  }
...
private:
  ...
  void addObjectClone(const std::string &oldName,
