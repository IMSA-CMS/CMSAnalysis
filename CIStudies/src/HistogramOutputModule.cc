...
HistogramOutputModule::HistogramOutputModule(
    const std::shared_ptr<GenSimIdentificationModule> genSimModule,
    const std::shared_ptr<RecoIdentificationModule> recoModule,
    const std::shared_ptr<WeightingModule> weightingModule,
    const std::shared_ptr<LRWeightModule> lrWeightModule)
    : genSim(genSimModule), reco(recoModule), weighting(weightingModule),
...
void HistogramOutputModule::addMassBinObject(std::string name,
                                             std::string massbin) {
  if (massBinMap.find(name) == massBinMap.end()) //
  {
    ...
    massBinMap[name].push_back(
        massbin); // Add the mass bin string to the vector
  }
}
...
void HistogramOutputModule::makeHistogram(const std::string &name,
  ...
  // std::cout << "HISTOGRAM:: (min, max) -> (" << min << ", " << max << ")\n";
...
bool HistogramOutputModule::process(const edm::EventBase &event) {
  ...
  for (auto hist : histograms) {
    bool draw =
        hist->shouldDraw(event); // call the shouldDraw function so we can call
                                 // process on the FilterModules
    ...
    if (isNewMassBin(massBin)) {
      ...
      //    // std::cout << "New Histogram Made: " << hist->getFilteredName() +
      //    massBin << '\n'; makeHistogram(hist->getFilteredName() + massBin,
      //    hist->getFilteredName() + massBin, hist->getNBins(),
    ...
    if (baseObjects.find(hist->getFilteredName() + massBin) ==
        baseObjects.end()) {
      // std::cout << "HISTOGRAM:: making histgoram for massbin: " << massBin
      //           << "\n";
      // std::cout << "Name: " << hist->getName() << '\n';
      // std::cout << "FilteredName: " << hist->getFilteredName() << '\n';
      // std::cout << "Mass Bin: " << massBin << '\n';
      // std::cout << "Histogram missing and made (with Mass Bin): " <<
      // hist->getFilteredName() + massBin << '\n';
      makeHistogram(hist->getFilteredName() + massBin,
                    hist->getFilteredName() + massBin, hist->getNBins(),
                    hist->getMinimum(), hist->getMaximum());
      addMassBinObject(hist->getFilteredName(), massBin);
    ...
    if (baseObjects.find(hist->getFilteredName()) == baseObjects.end()) {
      ...
      makeHistogram(hist->getFilteredName(), hist->getFilteredName(),
                    hist->getNBins(), hist->getMinimum(), hist->getMaximum());
    }
    ...
    if (draw) {
      // std::cout << "\nHistogram Filled: " << hist->getFilteredName() +
      // massBin
      //           << '\n'
      //           << "Value: " << hist->value() << '\n'
      //           << "Event weight: " << eventWeight << '\n';

      fillHistogram(hist->getFilteredName() + massBin, hist->value(),
                    eventWeight);
    }
  }
    ...
    for (auto massBin : massBins) {
      ...
      for (auto bin : pair.second) {
        ...
        if (bin == massBin.first && eventCount != 0) {
          // std::cout << "HISTOGRAM:: scale: " << massBin.second / eventCount
          //           << "\n";

          /*
          if (massBin.second != 0) {
            getHistogram(pair.first + bin)->Scale(1);
            // ->Scale(massBin.second /
          ...
          */
          ...
          // }
          getHistogram(pair.first)
              ->Add(getHistogram(pair.first + massBin.first));
        }
      }
    }
  }
