...
MatchingModule::MatchingModule(
    ...
    double deltaRCut)
    : genSim(genSimModule), reco(recoModule), deltaRCutoff(deltaRCut) {}
...
bool MatchingModule::process(const edm::EventBase &event) {
  ...
  std::vector<Particle> genSimParticles(
      genSim->getGenParticles().getParticles());
  std::vector<Particle> recoCandidates(
  ...
  while (!checkIsNull(genSimParticles) && !checkIsNull(recoCandidates)) {
      ...
      if (genParticle.isNotNull()) {
        ...
        for (auto &recoParticle : recoCandidates) {
          if (recoParticle.isNotNull()) {
            // Casting is bad form, but we're just going to have to live with
            // it. Rewriting the code is too much work Not checking the casts
            // because they have to work MatchingPair
            // pairCandidate(dynamic_cast<const
            ...
            // std::cerr << "I'd like to do small things like rule the world"
            //           << std::endl;
            // std::cerr << "Phi gen:" << pairCandidate.getGenParticle().phi()
            //           << std::endl;
            // std::cerr << "Eta gen:" << pairCandidate.getGenParticle().eta()
            //           << std::endl;
            // std::cerr << "pT gen:" << pairCandidate.getGenParticle().pt()
            //           << std::endl;

            // std::cerr << "Phi reco:" << pairCandidate.getRecoParticle().phi()
            //           << std::endl;
            // std::cerr << "Eta reco:" << pairCandidate.getRecoParticle().eta()
            //           << std::endl;
            // std::cerr << "pT reco:" << pairCandidate.getRecoParticle().pt()
            //           << std::endl;

            // if this delta R is better than the previous best one, keeps track
            // of the information by assigning values ot pariDataList
            if (pairCandidate.getDeltaR() < deltaRMin) {
              ...
              // std::cerr << "New pairCandidate assigned with DeltaR:"
              //           << pairCandidate.getDeltaR() << std::endl;
            }
          }
          ++matchingRecoCounter;
        }
    ...
    // makes an additional delta R cut and fills matching best pairs, resets
    // values for the next loop checks if the final (and best) delta R value for
    // the matches passes the cut
    if (deltaRMin < deltaRCutoff) {
      // std::cerr << "between if statements" << std::endl;
      // keeps track of that match by adding it to the vector that will be
      // returned
      if (pairDataList.getGenParticle().isNotNull() &&
          pairDataList.getRecoParticle().isNotNull()) {
        matchingBestPairs.addMatchingPair(pairDataList);
        // std::cerr << "New best match assigned // DeltaRMin: " << deltaRMin <<
        // std::endl;
      }
    }
    ...
    // else
    //{
    // std::cerr << "FAILED DELTA R CUTOFF // deltaR: " << deltaRMin <<
    // std::endl;

    //}
    // else
    // 	{
    // 	  std::cout << "DeltaR Cutoff FAILED! Gen Sim CS = " +
    // std::to_string(genSimCS) << std::endl;
    // 	}
  ...
  // 	  if (particle)
  // 	    {
  // 	      std::cout << "Unmatched Particles Eta: " << particle->eta() <<
  // std::endl;
  // 	    }
  // 	}
  //   }
