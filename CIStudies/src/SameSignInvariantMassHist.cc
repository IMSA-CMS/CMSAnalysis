...
SameSignInvariantMassHist::SameSignInvariantMassHist(const bool typeSwitch, const std::string& iname, int iNBins, double iminimum, double imaximum, 
  bool iUsingPhi, bool iMultipleMasses, std::shared_ptr<Selector> selector) :
...
std::vector<double> SameSignInvariantMassHist::protectedValue(bool typeGenSim) const
{
  if (multipleMasses)
  {
    ...
    {
      auto genParticles = getInput()->getLeptons(InputModule::RecoLevel::GenSim, leptonSelector);
    ...
    {
      auto recoParticles = getInput()->getLeptons(InputModule::RecoLevel::Reco, leptonSelector);
