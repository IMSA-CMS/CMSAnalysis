...
TwoInvariantMassesHist::TwoInvariantMassesHist(const std::string &iname, int iNBinsX, int iNBinsY, double iMinX, double iMinY, 
  double iMaxX, double iMaxY, bool useCorrectedLeptons) : 
  HistogramPrototype2D(iname, iNBinsX, iNBinsY, iMinX, iMinY, iMaxX, iMaxY), 
  useCorrectedLeptons(useCorrectedLeptons)
{}
...
ParticleCollection<Lepton> TwoInvariantMassesHist::getCorrectedLeptons() const
{
...
std::vector<std::pair<double, double>> TwoInvariantMassesHist::value2D() const
{
  auto leptons = useCorrectedLeptons ? getCorrectedLeptons() : getInput()->getLeptons(EventInput::RecoLevel::Reco);
  if (leptons.size() != 4)
  {
    return {};
  }
...
std::pair<double, double> TwoInvariantMassesHist::sortParticlePairs(const std::pair<Particle, Particle> &firstPair, const std::pair<Particle, Particle> &secondPair) const
{
  ...
  if (!((firstPair.first.getType() == secondPair.first.getType()) && firstPair.second.getType() == secondPair.second.getType()) && !((firstPair.first.getType() == secondPair.second.getType()) && firstPair.second.getType() == secondPair.first.getType()))
  {
    // Count number of electrons and muons in each pair
    int nElectrons1 = 0;
    int nMuons1 = 0;

    if (firstPair.first.getType() == ParticleType::electron())
      nElectrons1++;
    else if (firstPair.first.getType() == ParticleType::muon())
      nMuons1++;
    if (firstPair.second.getType() == ParticleType::electron())
      nElectrons1++;
    else if (firstPair.second.getType() == ParticleType::muon())
      nMuons1++;

    int nElectrons2 = 0;
    int nMuons2 = 0;
    if (secondPair.first.getType() == ParticleType::electron())
      nElectrons2++;
    else if (secondPair.first.getType() == ParticleType::muon())
      nMuons2++;
    if (secondPair.second.getType() == ParticleType::electron())
      nElectrons2++;
    else if (secondPair.second.getType() == ParticleType::muon())
      nMuons2++;
