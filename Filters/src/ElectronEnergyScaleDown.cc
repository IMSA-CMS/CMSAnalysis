...
void ElectronEnergyScaleDown::selectParticles(const EventInput* input, Event& event) const
{
    ...
    for (const auto& particle : particles)
    {
        if (particle.getType() == ParticleType::electron()) 
		{
            ...
            )
            {
                ...
                auto sf = particle.getInfo("eScaleUp") != 0 ? particle.getInfo("eScaleUp") : particle.getInfo("eSigmaDown");
